...
/**
 * one gNB and one eNB + UE 
 * 
 */

NS_LOG_COMPONENT_DEFINE ("Test_SINR");
...
PrintPosition (Ptr<Node> node)
{
  ...
  NS_LOG_INFO ("Position +****************************** " << model->GetPosition () << " at time "
...
main (int argc, char *argv[])
{
  ...
  LogComponentEnable ("Test_SINR", LOG_LEVEL_INFO);
  ...
  enbPositionAlloc->Add (centerPosition);
  if (nMmWaveEnbNodes > 1)
  {
    for (int8_t i = 0; i < nMmWaveEnbNodes-1; i++)
    {
      enbPositionAlloc->Add (Vector ( speed*i, centerPosition.y, 3));
    }
  }
  ...
  AnimationInterface::SetConstantPosition (allEnbNodes.Get (0), 10, 10); 
  AnimationInterface::SetConstantPosition (allEnbNodes.Get (1), 20, 10); 
  AnimationInterface::SetConstantPosition (allEnbNodes.Get (2), 30, 10); 
  AnimationInterface::SetConstantPosition (allEnbNodes.Get (3), 40, 10); 
  AnimationInterface::SetConstantPosition (allEnbNodes.Get (4), 50, 10); 


  MobilityHelper ueMobility;
  ueMobility.SetMobilityModel ("ns3::ConstantVelocityMobilityModel");
  ueMobility.Install (ueNodes);
  ueNodes.Get (0)->GetObject<MobilityModel> ()->SetPosition (Vector (0, maxYAxis / 2, 0));
  ueNodes.Get (0)->GetObject<ConstantVelocityMobilityModel> ()->SetVelocity (Vector (speed, 0, 0));
  if (nUeNodes > 1)
  {
    for (int8_t i = 0; i < nUeNodes-1; i++)
    {
      ueNodes.Get(i+1)->GetObject<MobilityModel> ()->SetPosition (Vector (maxXAxis / 2+ 10*cos ((2 * M_PI * i) / (nUeNodes)), maxYAxis / 2+10*sin ((2 * M_PI * i) / (nUeNodes)), 3));
      ueNodes.Get(i+1)->GetObject<ConstantVelocityMobilityModel> ()->SetVelocity (Vector (0, 0, 0));
    }
  }
  ...
  double simTime = maxXAxis/speed; //doubleValue.Get ();
 ...
 AnimationInterface anim ("SNR_TEXT.xml"); // Mandatory

  for (uint32_t i = 0; i < lteEnbNodes.GetN (); ++i)
    {
      anim.UpdateNodeDescription (lteEnbNodes.Get (i), "eNB"); // Optional
      anim.UpdateNodeColor (lteEnbNodes.Get (i), 255, 0, 0); // Optional
    }
  for (uint32_t i = 0; i < mmWaveEnbNodes.GetN (); ++i)
    {
      anim.UpdateNodeDescription (mmWaveEnbNodes.Get (i), "gNB"); // Optional
      anim.UpdateNodeColor (mmWaveEnbNodes.Get (i), 0, 255, 0); // Optional
    }

  for (uint32_t i = 0; i < ueNodes.GetN (); ++i)
    {
      anim.UpdateNodeDescription (ueNodes.Get (i), "ue"); // Optional
      anim.UpdateNodeColor (ueNodes.Get (i), 0, 0, 25); // Optional
    }
  anim.EnablePacketMetadata (); // Optional
  anim.EnableIpv4RouteTracking ("routingtable-wireless.xml", Seconds (0), Seconds (simTime), Seconds (0.1)); //Optional
