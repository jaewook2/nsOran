...
main (int argc, char *argv[])
{
  ...
  double maxYAxis = 40;
  double uepx = maxXAxis/2+1;
  double uepy = maxYAxis/2+1;
  //double speed = 20;
  std::string fileName = "text.txt";

  // Command line arguments
  CommandLine cmd;
  cmd.AddValue("uepx", "Ue possition X", uepx);
  cmd.AddValue("uepy", "Ue possition Y", uepy);
  cmd.AddValue("fileName", "Userposition file name", fileName);
  cmd.Parse (argc, argv);
  ...
  Config::SetDefault ("ns3::MmWaveBearerStatsConnector::MmWaveSinrOutputFilename", StringValue (fileName));
  ...
  double isd = 20; // (interside distance)
  ...
  */
  ...
  //uePositionAlloc->Add (Vector ( uepx, uepy, 3));
  
  for (int8_t i = 0; i < nUeNodes; ++i)
    {
      x = uepx+i;
      y = uepy;
  ...
  MobilityHelper uemobility;
  enbmobility.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
  enbmobility.SetPositionAllocator (uePositionAlloc);
  enbmobility.Install (ueNodes);
  
  int index_offeNB = 1;
  for (int i = 0; i<nMmWaveEnbNodes; i++) {
    if (i==index_offeNB){
      Ptr<MmWaveEnbPhy> mmWavePhy = mmWaveEnbNodes.Get (i)->GetObject<MmWaveEnbNetDevice> ()->GetPhy ();
  ...
  double simTime = 0.5; //doubleValue.Get ();
