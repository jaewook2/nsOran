...
main (int argc, char *argv[])
{
 ...
 Config::SetDefault ("ns3::MmWaveEnbNetDevice::EnableE2MsgReporting",
  ...
  std::vector<double> centerFrequencies = {28e9, 29e9, 30e9, 31e9}; // 28, 29, 30, and 31 GHz
  double bandwidth = 20e6; // Bandwidth: 100 MHz for all gNBs  // Distance between the mmWave BSs and the two co-located LTE and mmWave BSs in meters
...
//  uint8_t nMmWaveEnbNodes =4;
  ...
  for (uint8_t i = 0; i < nMmWaveEnbNodes_test; ++i)
    {
      // Configure each gNB with a different center frequency and bandwidth
      //mmwaveHelper->SetAttribute ("CenterFreq", DoubleValue (centerFrequencies[i])); // Assign unique frequency
      Config::SetDefault ("ns3::MmWavePhyMacCommon::CenterFreq", DoubleValue (centerFrequencies[i]));
      // Install device on the gNB
      mmWaveEnbDevs.Add (mmwaveHelper->InstallEnbDevice (mmWaveEnbNodes.Get (i)));
    }
 /*
