...
void
PrintPosition (std::string filename, Ptr<Node> node)
{
  ...
  outFile << Simulator::Now ().GetSeconds () <<","<< pos.x << "," << pos.y << std::endl;
  ...
  for (int i = 0; i < numPrints; i++)
     {
       for (uint32_t j = 0; j < ueNodes.GetN (); j++)
         {
           Simulator::Schedule(MicroSeconds(500), &PrintPosition, "ue_trace.txt", ueNodes.Get (j));
         }
     }
